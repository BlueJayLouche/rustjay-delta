...
impl ControlGui {
    ...
    pub(super) fn build_midi_tab(&mut self, ui: &imgui::Ui) {
        ...
        if ui.collapsing_header("Motion", imgui::TreeNodeFlags::DEFAULT_OPEN) {
            ...
            if ui.button("Learn: Red Delay") {
                let mut state = self.shared_state.lock().unwrap_or_else(|e| e.into_inner());
                state.midi_command = MidiCommand::StartLearn {
                    param_path: "motion/red_delay".to_string(),
                    param_name: "Red Delay".to_string(),
                };
            }
            ui.same_line();
            ui.text_disabled("(CC: --)");

            if ui.button("Learn: Green Delay") {
                let mut state = self.shared_state.lock().unwrap_or_else(|e| e.into_inner());
                state.midi_command = MidiCommand::StartLearn {
                    param_path: "motion/green_delay".to_string(),
                    param_name: "Green Delay".to_string(),
                };
            }
            ui.same_line();
            ui.text_disabled("(CC: --)");

            if ui.button("Learn: Blue Delay") {
                let mut state = self.shared_state.lock().unwrap_or_else(|e| e.into_inner());
                state.midi_command = MidiCommand::StartLearn {
                    param_path: "motion/blue_delay".to_string(),
                    param_name: "Blue Delay".to_string(),
                };
            }
            ui.same_line();
            ui.text_disabled("(CC: --)");

            if ui.button("Learn: Intensity") {
                let mut state = self.shared_state.lock().unwrap_or_else(|e| e.into_inner());
                state.midi_command = MidiCommand::StartLearn {
                    param_path: "motion/intensity".to_string(),
                    param_name: "Intensity".to_string(),
                };
            }
            ui.same_line();
            ui.text_disabled("(CC: --)");
