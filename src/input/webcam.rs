...
impl WebcamCapture {
    ...
    pub fn start(&mut self) -> anyhow::Result<Receiver<WebcamFrame>> {
        ...
        let thread_handle = thread::spawn(move || {
            ...
            // Pre-allocate BGRA buffer outside loop to reuse across frames
            let bgra_capacity = (actual_width * actual_height * 4) as usize;
            let mut bgra_buf = Vec::with_capacity(bgra_capacity);
            ...
            loop {
                ...
                match camera.frame() {
                    Ok(frame) => {
                        ...
                        // Iterate the borrowed buffer directly — no .to_vec() copy needed.
                        // Reuse pre-allocated buffer (clear keeps the allocation).
                        bgra_buf.clear();
                        ...
                        let webcam_frame = WebcamFrame {
                            ...
                            // Swap out the filled buffer; replace with a fresh one that
                            // reuses the old allocation on the next iteration.
                            data: std::mem::replace(&mut bgra_buf, Vec::with_capacity(bgra_capacity)),
